...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  /* To store num. of argumnets. */
  int argc = -1;

  /* To use strtok_r. */
  char *str, *svptr, *fn_copy;
  ...
  /* Word alignment and put NULL. */
  *esp = (char *)(((uintptr_t)*esp & ~(0x3)) - 8);

  /* Put addr. of words after NULL. */
  for(i = argc; i--; *esp -= 4) *(char **)*esp = args[i];
  *(char **)*esp = *esp + 4;

  /* Put argc, return addr. */
  *(int *)(*esp - 4) = argc;
  *(uint32_t *)(*esp - 8) = 0x0;
  *esp -= 8;
